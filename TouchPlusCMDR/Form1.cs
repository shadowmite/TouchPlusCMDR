...
namespace TouchPlusCMDR
{
    public partial class Form1 : Form
    {
        ...
        Boolean filters = false;                                                    // Flag to hold value of filters in viewer class
        ...
        {
            // Get serial number
            // Unstable right now, needs to pull a unmanaged byte array. Might hack the stack out from under us!
            _TouchPlus.GetSerialNum();
            while (_TouchPlus.busy) ;                                           // Wait for the library to finish
            UpdateMessages();
            if (_TouchPlus.errors.Count != 0)
            {
        ...
        private void FiltersBN_Click(object sender, EventArgs e)
        {
            if (filters)
            {
                _Viewer.SetFilters(false);
                filters = false;
            }
            else
            {
                _Viewer.SetFilters(true);
                filters = true;
            }
        }
    }
